...
const generateRowKey = (
  ...
  isoId: string,
  sheetNumber: number,
): string => `${spoolId}-${isoId}-${sheetNumber}`;
...
export function useWeldTable(
  initialWeldItems: Weld[],
  ...
  const { processWeld, editWps, editFillerMaterial, isSubmitting } =
    useWeldOperations({
  ...
  const isometricItems = useMemo(() => {
    const spoolsMap = buildSpoolsMap(
      initialWeldItems,
      stateManagement.updatedWelds,
    );
  ...
  const weldItems = useMemo(() => {
    if (selectedSpool === null || !selectedIso || sheetNumber === null)
      return [];
  ...
  const getUpdatedWeldData = useCallback(
    (weldId: number): WeldItemWithSpool | null => {
  ...
  const handleNextWorkflow = useCallback(async () => {
    if (!selectedSpool || !selectedIso || sheetNumber === null || isSubmitting)
      return;

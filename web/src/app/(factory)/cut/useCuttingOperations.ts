...
export function useCuttingOperations({
  onSuccess,
  onError,
}: UseCuttingOperationsProps = {}) {
  ...
  const editHeatNumber = useCallback(
    async (item: PipeLength, heatNumber: number) => {
      ...
      return performOperation(
        API_ROUTES.pipeLength.editHeatNumber,
        {
          pipeLengthId: item.id,
          heatNumber: heatNumber.toString(),
        },

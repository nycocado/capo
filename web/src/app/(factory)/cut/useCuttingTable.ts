...
import {
  CuttingAction,
  CuttingState,
  UseCuttingTableCallbacks,
} from '@/app/(factory)/cut/useCuttingTable.types';
...
const cuttingReducer = (
  ...
  switch (action.type) {
    case 'toggleInfo': {
      ...
      const newAllState =
        currentState === 'information' ? 'initial' : 'information';
      ...
      const updatedWorkState =
        currentState === 'information'
          ? Object.fromEntries(
              Object.entries(state.workState).filter(
                ([key]) => Number(key) !== id,
              ),
            )
          : state.workState;
    ...
    case 'workClick': {
      ...
      if (
        currentWorkState === 'initial' ||
        currentWorkState === 'information'
      ) {
        const hasOtherActive = Object.entries(state.workState).some(
          ([key, state]) =>
            (state === 'information' || state === 'working') &&
            Number(key) !== id,
        );
...
const useEventHandlers = (
  ...
  const handleNextWorkflow = useCallback(() => {
    ...
    const workingItem = availableItems.find(
      (item) => workState[item.id] === 'working',
    );
    if (workingItem) {
    ...
    const infoItem = availableItems.find(
      (item) => workState[item.id] === 'information',
    );
    if (infoItem) {
  ...
  const areAllWorkingItemsFinished = useCallback(() => {
    ...
    const workingTabItems = items.filter(
      (item) =>
...
export function useCuttingTable(
  ...
  const rowStateAccessor = useRowStateAccessor(
    activeTab,
    allState,
    workState,
    workingIds,

...
const assemblyReducer = (
  state: AssemblyStateManagement,
  action: AssemblyAction,
): AssemblyStateManagement => {
  switch (action.type) {
    case 'setRowWorking': {
...
export function useAssemblyTable(
  initialItems: Joint[],
  ...
  const findJointIdForWeld = useCallback(
    (weldId: number): number | null => {
    ...
    () =>
      isometricItems.flatMap((item) =>
        item.sheets.map((sheet) => ({
          ...
          weldCount: sheet.spools.reduce(
            (total, spool) => total + spool.welds.length,
            0,
          ),
          spools: sheet.spools,
        })),
  ...
  const getRowState = useCallback(
    (isoId: string, sheetNumber: number): AssemblyRowState => {
  ...
  const getWeldState = useCallback(
    (weldId: number): AssemblyItemState => {
  ...
  const areAllWeldsFinished = useCallback(
    (isoId: string, sheetNumber: number): boolean => {
  ...
  const areAllWorkingRowsFinished = useCallback(() => {
    ...
    const workingTabRows = assemblyRows.filter((row) =>
      workingRows.some(
        (wr) => wr.isoId === row.isoId && wr.sheetNumber === row.sheetNumber,
      ),
    );
  ...
  const findIsometricId = useCallback(
    (isoId: string, sheetNumber: number): number | null => {
      const relevantJoint = initialItems.find((joint) =>
        joint.spool.revs.some(
          (rev) =>
  ...
  const handleListConsultation = useCallback(async () => {
    ...
    const isometricId = findIsometricId(
      selectedRow.isoId,
  ...
  const handleWeldClick = useCallback(
    async (item: any) => {
      if (!item?.id || !selectedIso || sheetNumber === null || isSubmitting)
        return;
      ...
      if (currentState === 'initial') {
        ...
        if (jointId) {
          ...
          callbacks?.onError?.(
            'Could not find joint information for this weld',
          );
        }
      } else {
  ...
  useEffect(() => {
    ...
    if (rowState === 'working') {
      ...
      setSelectedWeld(
        availableWeld || (weldItems.length > 0 ? weldItems[0] : null),
      );
  ...
  const rowStateAccessor = useCallback(
    (item: any) => getRowState(item.isoId, item.sheetNumber),
    [getRowState],
  );
    ...
    () => ({
      ...
      finished: {
        ...
        onClick: (item: any) => {
          if (selectedIso && sheetNumber !== null && !isSubmitting) {
            const rowState = getRowState(selectedIso.id, sheetNumber);
            const allFinished = areAllWeldsFinished(
              selectedIso.id,
              sheetNumber,
            );
...
function buildHierarchyMap(
  joints: Joint[],
): Map<string, Map<number, SheetSpools>> {

...
function extractMaterialsFromAssemblyList(assemblyList: AssemblyListDto): MaterialsResponse {
...
export function useAssemblyMaterialVerification() {
  ...
  const allPipeLengthsVerified = useMemo(() => {
    ...
    return materials.pipeLengths.every(
      (pipeLength) => state.pipeLengthStates[pipeLength.internalId] === 'finished',
    );
  ...
  const canGoToPrevious = useMemo(() => currentStep === 'fitting', [currentStep]);
  ...
  const startVerification = useCallback(async (assemblyList: AssemblyListDto, onComplete: () => void) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const extractedMaterials = extractMaterialsFromAssemblyList(assemblyList);
      setMaterials(extractedMaterials);
      setCurrentStep('pipeLength');
      setShowModal(true);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to extract materials');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openMaterialsConsultation = useCallback(async (assemblyList: AssemblyListDto) => {
    setLoading(true);
    setError(null);
  ...
  // Handle clicks
  const handlePipeLengthClick = useCallback((pipeLength: PipeLengthDto) => {
  ...
  const handleFittingClick = useCallback((fitting: FittingDto) => {
    if (isConsultationMode) return;
  ...
  // Get states
  const getPipeLengthState = useCallback((pipeLength: PipeLengthDto): MaterialState => {
  ...
  const getFittingState = useCallback((fitting: FittingDto): MaterialState => {
    if (isConsultationMode) return 'initial';

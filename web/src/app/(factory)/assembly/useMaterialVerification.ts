...
async function fetchMaterialsForIsometric(
  isometricId: number,
...
export function useMaterialVerification() {
  ...
  const startVerification = useCallback(
    async (isometricId: number, onComplete: () => void) => {
      setLoading(true);
      setError(null);
  ...
  const handlePipeLengthClick = useCallback(
    (pipeLength: PipeLength) => {
      ...
      if (pipeLength?.internalId) {
        dispatch({
          type: 'togglePipeLength',
          id: pipeLength.internalId,
        });
      }
    },
    [isConsultationMode],
  ...
  const getPipeLengthState = useCallback(
    (pipeLength: PipeLength): MaterialState => {

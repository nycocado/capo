...
export default function DashboardClient({ data }: { data: StatisticsData }) {
  const pieData = [
    {
      name: 'Finished',
      value: parseFloat(data.progress.avgProgress.toFixed(2)),
    },
    {
      name: 'To Do',
      value: parseFloat((100 - data.progress.avgProgress).toFixed(2)),
    },
  ];
        ...
        <Row className="mb-2">
          <Col>
            <h1 className="text-primary mb-2">
              {data.project.name}: {data.project.internalId} -{' '}
              {data.progress.avgProgress.toFixed(2)}%
            </h1>

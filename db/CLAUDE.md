...
`01-` and `02-` are mounted into the MariaDB container's init dir and applied **only on first boot of a fresh volume**. To re-apply schema/seed changes you must `npm run docker:down` (drops the volume) then `up`, or `npm run docker:rebuild`.

...
- `/socket.io/` → `capo-api:3002` (WebSocket upgrade headers set; `proxy_read_timeout 3600s` — WS connections are long-lived and would be dropped at the default 60s)
- `/` → `capo-web:3000`
...
`/api/` and `/` use `Connection ""` (empty) — required for HTTP/1.1 keepalive to the upstream; only `/socket.io/` sends the WebSocket upgrade.

...
- **e2e** (`test/*.e2e-spec.ts`, `test/jest-e2e.json`, **ts-jest**) — boots the real app via `@nestjs/testing` against a throwaway `capo_test` schema (`test/global-setup-e2e.ts` re-applies `db/*.sql`); covers auth (login/me/guard) and the **cut→assembly gating**. ts-jest here (not swc) because the ORM needs `emitDecoratorMetadata`, which the tsc-style emit tolerates through the entity cycle (swc doesn't); `createMikroOrmConfig` sets `preferTs` outside production so discovery uses `entitiesTs`. The DB connection is read from `DATABASE_*` env (CI injects the service container; locally point it at the compose MariaDB).
